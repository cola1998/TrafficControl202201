...
def generate_routefile(file_name,N, maxStep,lturnRatio,sRatio):
    if lturnRatio+sRatio > 9:
        print("生成文件失败，请重新检查车流转向比例！lturnRatio+sRatio = ",lturnRatio+sRatio)
        return False
    ...
    with open(file_name, 'w') as routes:
        ...
        for car_counter, step in enumerate(car_gen_steps):
            ...
            if straight_or_turn < sRatio*0.1:  # 50%机会直行
                ...
                if route_straight == 1:
                    ...
                    print(
                        ...
                        car_counter, step), file=routes)
            elif straight_or_turn < (sRatio+lturnRatio)*0.1:  # 25%左转
...
# if __name__ == "__main__":
    # l_N = 3000  # 低车流量
    # l_file_name = 'high_net_3000.rou.xml'
    # MaxStep = 7200
    # # 不同的转弯比例
    # L = 5
    # sRatio = 4
    # generate_routefile(l_file_name,l_N, MaxStep,L,sRatio)
